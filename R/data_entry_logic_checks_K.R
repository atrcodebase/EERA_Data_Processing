...
## Timetable Year --------------------------------------------------------------
all_school_op_and_op_other_grades = clean_data.tool1$School_Operationality |>
    select(Site_Visit_ID, PARENT_KEY, Grade) |>
    group_by(Site_Visit_ID, PARENT_KEY, Grade) |>
    mutate(sheet = "School_Operationality", Grade_ID_v = paste0(Site_Visit_ID,"-",Grade)) |>
    filter(!is.na(Grade) & Grade != "" & !(duplicated(Grade_ID_v, fromLast = T) | duplicated(Grade_ID_v, fromLast = F))) |>
    ungroup() |>
    arrange(Grade_ID_v) |> pull(Grade_ID_v) |> unique()


all_school_op_and_op_other_grades_no_operational = clean_data.tool1$School_Operationality |>
  select(Site_Visit_ID, PARENT_KEY, Grade, is_operational = C13A1) |>
  mutate(sheet = "School_Operationality", Grade_ID_v = paste0(Site_Visit_ID,"-",Grade)) |>
  filter(is_operational == "No" & !is.na(Grade) & Grade != "" & !(duplicated(Grade_ID_v, fromLast = T) | duplicated(Grade_ID_v, fromLast = F))) |>
  arrange(Grade_ID_v) |> pull(Grade_ID_v) |> unique()
