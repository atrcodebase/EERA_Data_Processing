...
not_qaed_ints_ps <- rbind(
  raw_data.tool1$data |> filter(!KEY %in% qaed_ints_key_ps ) |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 1 - Headmaster"),
  raw_data.tool2$data |> filter(!KEY %in% qaed_ints_key_ps ) |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 2 - Light"),
  raw_data.tool3$data |> filter(!KEY %in% qaed_ints_key_ps ) |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 3 - Headcount"),
  raw_data.tool4$data |> filter(!KEY %in% qaed_ints_key_ps ) |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 4 - Teacher"),
  raw_data.tool5$data |> filter(!KEY %in% qaed_ints_key_ps ) |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 5 - WASH"),
  raw_data.tool6$data |> filter(!KEY %in% qaed_ints_key_ps & Sample_Type == "Public School") |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 6 - Parent"),
  raw_data.tool7$data |> filter(!KEY %in% qaed_ints_key_ps & Sample_Type == "Public School") |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 7 - Shura")
) |>
...
inconsistent_qa_status_ps <- rbind(
  raw_data.tool1$data |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 1 - Headmaster"),
  raw_data.tool2$data |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 2 - Light"),
  raw_data.tool3$data |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 3 - Headcount"),
  raw_data.tool4$data  |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 4 - Teacher"),
  raw_data.tool5$data  |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 5 - WASH"),
  raw_data.tool6$data |> filter(Sample_Type == "Public School") |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 6 - Parent"),
  raw_data.tool7$data  |> filter(Sample_Type == "Public School") |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 7 - Shura")
) |>
  mutate(
...
not_qaed_ints_cbe <- rbind(
  raw_data.tool6$data |> filter(!KEY %in% qaed_ints_key_cbe & Sample_Type == "CBE") |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 6 - Parent"),
  raw_data.tool7$data |> filter(!KEY %in% qaed_ints_key_cbe & Sample_Type == "CBE") |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 7 - Shura"),
  raw_data.tool8$data |> filter(!KEY %in% qaed_ints_key_cbe) |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 8 - Class"),
  raw_data.tool9$data |> filter(!KEY %in% qaed_ints_key_cbe) |> select(any_of(meta_cols), KEY) |> mutate(tool = "Tool 9 - IP")
) |>
...
inconsistent_qa_status_cbe <- rbind(
  raw_data.tool6$data |> filter(Sample_Type == "CBE")  |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 6 - Parent"),
  raw_data.tool7$data |> filter(Sample_Type == "CBE") |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 7 - Shura"),
  raw_data.tool8$data |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 8 - Class"),
  raw_data.tool9$data |> select(any_of(meta_cols), KEY, review_status.data = review_status) |> mutate(tool = "Tool 9 - IP")
) |>
  mutate(
    review_status.data = case_when(
      review_status.data == "AWAITING_REVIEW" ~ "PENDING",
