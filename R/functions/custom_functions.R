...
check_select_multiple <- function(data, multi_vars, question_separator, choice_seprator, KEY="KEY"){
  ...
  for(question in multi_vars){
    # index_of_question = grepl(paste0("^", question, "+"), names(data))
    index_of_question = grepl(paste0("^", question, question_separator, "\\d|^", question,"$"), names(data))
    if(question %in% names(data) & any(index_of_question)) {
      print(paste0("Checking: ", question))
      # Get all series columns
      # series_cols <- names(select(data, starts_with(paste0(question, question_separator))))
      # changed by KO
      series_cols <- names(data)[index_of_question]
      
      data_sub <- data %>% 
        select(all_of(series_cols), KEY) %>% 
        filter(!is.na(get(question)) & get(question) != "")
      
      data_sub <- data_sub[data_sub[[question]] != "" & !is.na(data_sub[[question]]), ]
      
      if (nrow(data_sub) > 0) {
        for(i in 1:nrow(data_sub)){
          #question value
          val <- str_split(data_sub[[question]][i], choice_seprator)[[1]]
          #make related series column name
          series_columns <- paste0(question,question_separator, val)
          other_columns <- names(data_sub)[!names(data_sub) %in% c(series_columns, question, "KEY")]
          if(!all(series_columns %in% names(data_sub))){
            ...
            log <- c(data_sub$KEY[i], 
                     ...
                     Remarks = "Inconsistent series columns")
            series_log <- rbind(series_log, log)
...
reorder_choices_v2 <- function(data, questions, default_value){
  ...
  data_reshaped <- data_reshaped %>% 
    group_by(KEY) %>% 
    mutate(values = c(values[!values %in% default_value], values[values %in% default_value])) %>% 
    # # Re-add the Ranks
...
missing_translation <- function(data, KEY, excluded_cols){
  ...
  data_cols <- names(data)[!names(data) %in% excluded_cols]
...
labeler <-function (data, survey_label = "label:English", choice_lable = "label:English", 
                    multi_response_sep = ";", kobo_tool.survey, kobo_tool.choices) 
{
  # survey_questions <- read_excel(tool, "survey")
  # survey_choices <- read_excel(tool, "choices")
  
  survey_questions <- kobo_tool.survey
  survey_choices <- kobo_tool.choices
